...
For Codex, do not rely on the session itself to hold a 4-hour wait loop. Agent Brain keeps the waiting state and stores the response for later recovery.`;
}
...
export PATH="${AGENT_BRAIN_BIN_DIR}:/usr/local/bin:/opt/homebrew/bin:$PATH"
cd ${escapeShell(cwd)}
